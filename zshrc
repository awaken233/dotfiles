...
rgfzf() {
  ...
  local CHOICE=$(fzf --bind "change:reload:$RG_PREFIX {q} || true" \
    --sort \
    --multi \
    --preview '[[ ! -z {} ]] && filename=$(echo {} | cut -d: -f1) && line_number=$(echo {} | cut -d: -f2) && bat --color=always --style=numbers --highlight-line "$line_number" "$filename"' \
    --ansi --phony --query "$INITIAL_QUERY" \
    --prompt "字面量搜索> " \
    --header "F2:切换到正则模式 | Enter:跳转到目标行" \
    --bind "f2:change-prompt(正则搜索> )+reload(rg --line-number --column --smart-case --color=always {q} || true)+change-preview([[ ! -z {} ]] && filename=\$(echo {} | cut -d: -f1) && line_number=\$(echo {} | cut -d: -f2) && bat --color=always --style=numbers --highlight-line \"\$line_number\" \"\$filename\")")
